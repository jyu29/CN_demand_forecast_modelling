...
from src.outputs_stacking import read_format_forecast, compute_stacking
...
class ReadFormatStackingDataTests:
    @patch('src.outputs_stacking.read_multipart_parquet_s3', read_parquet_mocked)
    @patch('src.outputs_stacking.read_jsonline_s3', read_jsonline_mocked)
    @patch('src.outputs_stacking.from_uri', identity)
    def test_nominal(self):

        cutoff = 202050
        df_jobs = pd.read_parquet(DF_JOBS_PATH)
        df_jobs_cutoff = df_jobs[df_jobs['cutoff'] == cutoff].copy()
        deepar, arima, _ = read_format_forecast(df_jobs_cutoff, 
                                                short_term_algorithm='deepar', 
                                                long_term_algorithm='arima')
